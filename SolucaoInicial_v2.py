# -*- coding: utf-8 -*-
"""
Created on Wed Jan 30 21:16:05 2019
...
def SolucaoInicial(casename):
 ...
 #  Seq_Navio = data['Seq_Navio'].tolist()
  ...
  # N_Mov=data['value'][0][0]
    ...
    #------------------------------------------------------------#
    nvar = 0 
    model,vnames,nvar,startVar,startVal = variavel_v(model,S,N,T,nvar,omega,startVar,startVal)        
    model,qnames,nvar,startVar,startVal = variavel_q(model,N,R,C,nvar,q_o,q_d,q_r,q_c,startVar,startVal) 
    model,unames,nvar,startVar,startVal = variavel_u(model,N,R,C,nvar,Seq_Navio_Inv,startVar,startVal)
    model,wnames,nvar,startVar,startVal = variavel_w(model,N,R,C,nvar,w_o,w_d,w_a,w_r,w_c,startVar,startVal)
    model,znames,nvar,startVar,startVal = variavel_z(model,omega,N,T,R,C,S,Seq_Navio_Id_Inv,nvar,startVar,startVal)
    model,ynames,nvar,startVar,startVal = variavel_y(model,omega,Patios,S,N,H,W,T,nvar,startVar,startVal)
    model,bnames,nvar,startVar,startVal = variavel_b(model,omega,Patios,S,N,H,W,T,nvar,startVar,startVal) 
    model,xnames,nvar,startVar,startVal = variavel_x(model,omega,N,H,W,T,nvar,startVar,startVal)
 ...
 #   print('Tamanho startVal = ', restricoes2)
   ...
   # model.MIP_starts.write("SolucaoInicial_Instancia_1.mst")
    #model.MIP.strategy.file(3)
    model.parameters.mip.strategy.file.set(2)
   # model.parameters.workdir.set()
   # model.parameters.mip.limits.treememory.set(2e+75)
    model.parameters.mip.strategy.variableselect.set(3)  #Another memory-conserving strategy is to use strong branching for variable selection; that is, set the MIP variable selection strategy parameter (VarSel, CPX_PARAM_VARSEL)
                                                         # to the value 3. Strong branching requires substantial computational effort at each node to decide the best branching variable. 
                                                         #As a result, it generates fewer nodes and thus makes less overall demand on memory. Often, strong branching is faster as well.
    model.parameters.emphasis.memory.set(1) #On; conserve memory where possible                                                     
...
#    Pt = obterP(model,H[0],W[0],omega[0],T[0])
    ...
    Y = 'Instancia: %s <br> Solution status: %s  <br> Valor da Funcao Objetivo: %s  <br> Duracao: %s <br>' %(casename,status,fobj,solvetime)
...
def variavel_v(model,S,N,T,nvar,omega,startVar,startVal):
  ...
  #  model.MIP_starts.add(cplex.SparsePair(ind = vnames, val = val), model.MIP_starts.effort_level.repair, "first")               
    ...
    return model,vnames,nvar,startVar,startVal
...
def variavel_q(model,N,R,C,nvar,q_o,q_d,q_r,q_c,startVar,startVal):
   ...
   # model.MIP_starts.add(cplex.SparsePair(ind = qnames, val = val), model.MIP_starts.effort_level.repair, "first")    
    ...
    return model,qnames,nvar,startVar,startVal
...
def variavel_u(model,N,R,C,nvar,Seq_Navio_Inv,startVar,startVal):
    ...
    return model,unames,nvar,startVar,startVal
...
def variavel_w(model,N,R,C,nvar,w_o,w_d,w_a,w_r,w_c,startVar,startVal):
   ...
   # model.MIP_starts.add(cplex.SparsePair(ind = wnames, val = val), model.MIP_starts.effort_level.repair, "first") 
    ...
    return model,wnames,nvar,startVar,startVal
...
def variavel_z(model,omega,N,T,R,C,S,Seq_Navio_Id_Inv,nvar,startVar,startVal):
   ...
   # model.MIP_starts.add(cplex.SparsePair(ind = znames, val = val), model.MIP_starts.effort_level.repair, "first")   
    ...
    return model,znames,nvar,startVar,startVal
...
def variavel_y(model,omega,Patios,S,N,H,W,T,nvar,startVar,startVal):
   ...
   # model.MIP_starts.add(cplex.SparsePair(ind = ynames, val = val), model.MIP_starts.effort_level.repair, "first")   
    ...
    return model,ynames,nvar,startVar,startVal
...
def variavel_b(model,omega,Patios,S,N,H,W,T,nvar,startVar,startVal):
   ...
   # model.MIP_starts.add(cplex.SparsePair(ind = bnames, val = val), model.MIP_starts.effort_level.repair, "first")   
    ...
    return model,bnames,nvar,startVar,startVal
#------------------------------------------------------------------------------------------------------------------#
#------------------------------------------------------------------------------------------------------------------# 
def variavel_x(model,omega,N,H,W,T,nvar,startVar,startVal):
    ...
    return model,xnames,nvar,startVar,startVal
#------------------------------------------------------------------------------------------------------------------#
#------------------------------------------------------------------------------------------------------------------# 
...
_ = SolucaoInicial('SolucaoInicial_Aleatoria_Instancia_1.mat')